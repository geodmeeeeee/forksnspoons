...
var soft = {
	...
	navigator: function() {
		...
		// Language
		soft.navigatorInfo.language = navigator.language;
	...
	start: function() {
		...
		if (softConfig.userSelect == false) {
			$('body#soft').addClass('no-user-select');
		}

		// User Drag
		if (softConfig.userDrag == false) {
			$('body#soft').addClass('no-user-drag');
		}
		...
		document.head.innerHTML += '<!-- Head JS Files/Scripts -->';
		for (var i=0; i<softConfig.headJsScripts.length; i++) {
			...
			if (softConfig.headJsScripts[i].src != '' && softConfig.headJsScripts[i].src != undefined)
				headJsScript.src = softConfig.headJsScripts[i].src;
			if (softConfig.headJsScripts[i].script != '' && softConfig.headJsScripts[i].script != undefined)
			headJsScript.innerHTML = softConfig.headJsScripts[i].script;
			document.head.appendChild(headJsScript);
		}
		document.head.innerHTML += '<!-- -->';
		...
		$('body#soft').addClass(soft.navigatorInfo.device + ' ' + soft.navigatorInfo.platform + ' ' + soft.navigatorInfo.OSName + ' ' + soft.navigatorInfo.OSVersion + ' ' + soft.navigatorInfo.browserName + ' ' + soft.navigatorInfo.browserVersion + ' ' + soft.navigatorInfo.appCodeName + ' ' + soft.navigatorInfo.appName + ' ' + soft.navigatorInfo.appVersion + ' ' + soft.navigatorInfo.product).attr('lang', soft.navigatorInfo.language.toLowerCase());
		...
		for (var i=0; i<softConfig.bodyJsScripts.length; i++) {
			var bodyJsScript = document.createElement('script');
			bodyJsScript.type = 'text/javascript';
			if (softConfig.bodyJsScripts[i].src != '' && softConfig.bodyJsScripts[i].src != undefined)
				bodyJsScript.src = softConfig.bodyJsScripts[i].src;
			if (softConfig.bodyJsScripts[i].script != '' && softConfig.bodyJsScripts[i].script != undefined)
			bodyJsScript.innerHTML = softConfig.bodyJsScripts[i].script;
			document.body.appendChild(bodyJsScript);
		}
		document.body.innerHTML += '<!-- -->';
	...
	orientationScreen: function() {

		var orientation = 'horizontal';

		if (window.innerWidth < window.innerHeight) orientation = 'vertical';

		$('body#soft').removeClass('horizontal-orientation').removeClass('vertical-orientation').addClass(orientation + '-orientation');
	...
	pageBuild: function(page, position) {
		...
		var pageIndex, pageId, pageClass, pageAttribute, pageFilePath, pageActive, pageVarBase, pageContent, pageOverlayContent;
		...
		if (page == undefined) {
			...
			if (isNaN(page)) {
				...
				if (page > softContent[soft.languageIndex].contentPages.length) {
				...
				if (position != undefined && position != 'prepend' && position != 'append') {
					...
					if ($('body#soft > main > section#soft-pages #' + pageId).length == 0) {
						...
						if (pageActive == false) {
							...
							pageContent = softContent[soft.languageIndex].contentPages[pageIndex].pageContent;

							pageOverlayContent = softContent[soft.languageIndex].contentPages[pageIndex].pageOverlayContent;
	...
	pageShow: function(pageIndex) {
		...
		soft.pageContent = softContent[soft.languageIndex].contentPages[pageIndex].pageContent;
		soft.pageOverlayContent = softContent[soft.languageIndex].contentPages[pageIndex].pageOverlayContent;
	...
	toScale: function() {
		...
		$('.soft-scaled').each(function(){
			...
			var widthScale = ((resizedWidth / initHandlerWidth) * 100) / 100;
			var heightScale = ((resizedHeight / initHandlerHeight) * 100) / 100;
			...
			}).attr('current-scale', heightScale);
		});
	...
	pageLoader: function(pageIndex) {
		...
		if (totalPageFiles > 0) {
			...
			$('body#soft > main').append('<div id="soft-loader"><div><i class="soft-scaled" initial-width="100" initial-height="100"></i></div></div></main>');
			soft.toScale();
			...
			$.html5Loader({
				...
				onUpdate: function (percentage) {
					...
					$('body#soft > main > #soft-loader > div > i').html(soft.percentPageLoaded);
					if (percentage >= 100) {}
				},
