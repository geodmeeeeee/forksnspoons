class Player {
    ...
    getAttack() {
    let multiplier = 0.5 + 0.5 * this.currentStamina;

    // Nếu cầu thủ này đã được bạn mua (thuộc đội của bạn)
    if (this.isBought()) {
        multiplier = 999999999999999999; // Buff riêng cho gà nhà
    }

    return this.getBaseAttack().mul(multiplier);
    }

    getDefense() {
        // Nếu cầu thủ thuộc đội của bạn, phòng thủ x999999999999999999
        if (this.isBought()) {
            return this.getBaseDefense().mul(999999999999999999);
    }
        // Đối thủ vẫn dùng công thức cũ
